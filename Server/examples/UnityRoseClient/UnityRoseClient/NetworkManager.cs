...
namespace Network
{
    ...
    public class NetworkManager
    {
        ...
        private static void RecieveCallback(IAsyncResult ar)
        {
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
						Console.WriteLine("Recieved " + state.sb.ToString());
                    }

                    string clearText = AES.Decrypt(state.sb.ToString());
        ...
        public static void Send(Packets.IPacket packet)
        {
			...
			byte[] byteData = Encoding.ASCII.GetBytes( AES.Encrypt(packet.ToString()));
        ...
        private static void SendCallback(IAsyncResult ar)
        {
            try
            {
				...
				Console.WriteLine ("Send " + bytesSend + " bytes to server.");
            }
            catch (Exception e)
            {
				Console.WriteLine (e.ToString());
